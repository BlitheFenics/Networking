...
void Alab7Character::Tick(float DeltaSeconds)
{
	Super::Tick(DeltaSeconds);

	CurrentHealth -= 0.5f * DeltaSeconds;
}
...
void Alab7Character::OnHealthUpdate()
{
	if (IsLocallyControlled())
	{
		FString msg = FString::Printf(TEXT("Health Updated: %f/%f"), CurrentHealth, MaxHealth);
		//GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Blue, msg); //include gengine header?
...
void Alab7Character::SetCurrentHealth(float NewHP)
{
	//If the new health is different AND this machine has net authourity
	if (!FMath::IsNearlyEqual(NewHP, CurrentHealth) && Role == ROLE_Authority) //Or HasAuthority()
	{
		CurrentHealth = FMath::Clamp(NewHP, 0.f, MaxHealth);
		OnHealthUpdate();
	}
}
